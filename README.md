...
- `link`: This table stores survey answers which are links to external websites and metadata. Along with `url` itself, a `questionId` and 3 metadata fields for key information are included, the title of each field is dependent on the displayTypeId. ID=1 => Title (`field1`), Author(`field2`), and Date of publication(`sourceDate`). ID=2 => Physician(`field1`), Type of Scan(`feild2`), and Date of Scan(`sourceDate`). To add additional display types, just create a new ID. `field1` and `field2` are the 2 metadata fields. `sourceDate` is the date the user viewed the link. Adding a new display type should not require a new enum and a rebuild, so an integer type was used for `displayTypeId`.
